...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091104167.png" alt="image-20230223091104167" style="zoom:50%;" />
...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091240104.png" alt="image-20230223091240104" style="zoom:50%;" />
...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091301357.png" alt="image-20230223091301357" style="zoom:50%;" />
...
[<img src="https://s1.ax1x.com/2022/11/27/zNOuDg.png" alt="zNOuDg.png" style="zoom:50%;" />](https://imgse.com/i/zNOuDg)

### DAC1138的增益和偏置调节电路

<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091446420.png" alt="image-20230223091446420" style="zoom:50%;" />

各种输出电压范围的接法：（引脚）46接69时，DAC为双极性输出；46接34时，DAC为单极性输出。

<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091507491.png" alt="image-20230223091507491" style="zoom:67%;" />
...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091529594.png" alt="image-20230223091529594" style="zoom:50%;" />
...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091546292.png" alt="image-20230223091546292" style="zoom:50%;" />

开关电流源有发射极耦合电路构成，因而速度很快
...
一种高速数模转换器的译码电流开关组合方法

![image-20230223091612813](https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091612813.png)

​     主数据流一分为二，分别进入A道和B道锁存器。

​     每道锁存器的高4位数据分别进入温度计译码器A和B，得到两组15位温度译码输出，两个锁存器2各输出21位数据进入多路转接器，转接器的输出控制电流开关，最后得到互补的电流输出。

$\mathrm{D}_{\mathrm{A}}$ 和 $\mathrm{D}_{\mathrm{B}}$ 同时输入而 $v_{\mathrm{CP} 2}$ 略迟于 $v_{C P 1}$ 锁存器2所对应的两部分数据打入时间正好 对应 $v_{\mathrm{CP} 2}$ 的上升沿和下降沿，经过多路转 接器后电流开关的动作就和 $\boldsymbol{v}_{C P}$ 完全相同， 而输出数据则交替地取自 $D_A$ 和 $D_B$

高速数模转换器的快速译码器部分，其基本电路如图所示

<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223091937578.png" alt="image-20230223091937578" style="zoom:50%;" />

可以采用多电平发射极耦合逻辑

$A$ 为高电平， $B_M$ 为低电平，输出 $C$ 为高电平，逻辑 关系为 $\mathrm{C}=\mathrm{A}\cdot \overline{B_M}$

这种开关速度很高，且不必处理肖特基垒势，不会因为二极管的正向电压降而影响开关速度



高速数模转换器的输出级部分

<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223092114428.png" alt="image-20230223092114428" style="zoom:50%;" />

分为数字部分和模拟部分，两部分除了有互补式信号联系外，电源、地、偏置电压全部分开

时钟信号$v_{cp1}$ 和$v_{cp2}$控制多路转接器以选通A道还是B道数据，放大器1是数字缓冲放大器，放大器2是模拟缓冲放大器，其接地是分开的，偏压也分开。

输出级电流开关的偏置电压$V_{OB}$与放大器的$V_{BD}$和$V_{BA}$也分开，以免数字开关噪声馈入模拟电路
...
<img src="https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223092328995.png" alt="image-20230223092328995" style="zoom:50%;" />

### DACx0502

双电压输出数模转换器 (DAC) 是高度精确的低功耗器件，具有电压输出。DACx0502的精度高，封装小，因此非常适合用于增益和失调校准、电流或电压设定点生成以及电源控制等应用。

![image-20230223092402381](https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223092402381.png)
...
![image-20230223092452917](https://mypic-1312707183.cos.ap-nanjing.myqcloud.com/image-20230223092452917.png)
